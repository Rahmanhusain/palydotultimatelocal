...
//signup and Login input refs
  ...
  const handlePasswordChange = (e) => {
    ...
    setIsMatching(newPassword === confirmPassword || !newPassword || !confirmPassword);
  };

  const handleConfirmPasswordChange = (e) => {
    const newConfirmPassword = e.target.value;
    setConfirmPassword(newConfirmPassword);
    setIsMatching(password === newConfirmPassword || !password || !newConfirmPassword);
  };
    ...
    <div className="container">
      ...
      <form id="signup-form" ref={signUpformRef} className={isLogin ? 'hide' : ''} onSubmit={(e)=>handleSignupAndLoginFormSubmit(e,"signup")}>
        ...
        <div className="form-group" style={{ position: 'relative' }}>
          ...
          <i id="signupeyeconfirm" className={showPasswordSignconfirm ? 'fa-solid fa-eye-slash' : 'eye-icon fas fa-eye'} onClick={togglePasswordVisibilitySignConf}></i>
        </div>
        <button type="submit" className="submit" disabled={!isMatching}>Sign Up</button>
      </form>
      {/* signup Form end */}
      ...
      <form ref={loginFormRef} id="login-form" className={isLogin ? '' : 'hide'} onSubmit={(e)=>handleSignupAndLoginFormSubmit(e,ischecked?'LoginOTP':'Login')}>
        ...
        <div id='checkdiv'>
          <input type="checkbox" checked={ischecked} onChange={handleCheckboxChange} name="LoginOTP" id="LoginOTP"/>
          <label htmlFor="LoginOTP">Login Via OTP</label>
          </div>
        <div id='forgetdiv'></div><p><a href=''>forget password ?</a></p>
        </div>
        
        <button type="submit" className="submit">{ischecked?"Send OTP":"Login"}</button>
      </form>
      {/* Login Form end */}
     ...
     (
      ...
      <h3 id='crossOTP'><i className="fa-solid fa-xmark" onClick={exitOTP}></i></h3>
      <header id="otpnheader">
       {timer > 0 ? (
          <h3>{timer}s</h3>
        ) : (
          <p><i className="fa-solid fa-arrows-rotate" onClick={handleOTPResend}></i><br />RESEND OTP</p>
        )}
      </header>
